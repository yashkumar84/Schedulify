    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
                const portals = document.getElementsByClassName('multi-select-portal');
    ...
    return (
        <div className={`relative ${className}`} ref={containerRef}>
            ...
            <div
                ...
                className={`
                    w-full min-h-[48px] flex flex-wrap items-center gap-2 px-3 py-2 bg-white border border-secondary-200 rounded-2xl
            ...
            >
                {selectedOptions.length > 0 ? (
                    selectedOptions.map(opt => (
                        <div
                            key={opt.id}
                            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-lg ${opt.bg || 'bg-primary-50'} ${opt.color || 'text-primary-700'} text-xs font-bold border border-transparent hover:border-current/20 transition-all`}
                        >
                    ...
                    <span className="text-sm font-semibold text-secondary-400 pl-1">{placeholder}</span>
                )}

                <div className="flex-1 min-w-[20px]" />
                <ChevronDown
                    size={18}
                    className={`text-secondary-400 transition-transform duration-500 ease-out flex-shrink-0 ${isOpen ? 'rotate-180 text-primary-500' : ''}`}
                />
            </div>

            {isOpen && coords && createPortal(
                <div
                    className="multi-select-portal"
                    style={{
                        position: 'fixed',
                                ...
                                filteredOptions.map((option) => {
                                    ...
                                    return (
                                        <div
                                            ...
                                            `}
                                        >
                                            <div className={`p-2 rounded-lg transition-transform duration-300 group-hover:scale-110 ${isSelected ? (option.bg || 'bg-primary-100') : (option.bg || 'bg-secondary-100')} ${isSelected ? (option.color || 'text-primary-600') : (option.color || 'text-secondary-600')}`}>
